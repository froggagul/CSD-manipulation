...
from baselines.her_music.util import convert_episode_to_batch_major, store_args
...
class RolloutWorker:
    ...
    def generate_rollouts(self, generated_goal=False, z_s_onehot=False):
        ...
        if self.render == 'rgb_array':
            imgs = np.empty([self.rollout_batch_size, self.T, 500, 500, 3])
        ...
        for t in range(self.T):
            ...
            for i in range(self.rollout_batch_size):
                try:
                    ...
                    if self.render:
                        if self.render == 'rgb_array':
                            imgs[i][t] = self.envs[i].render(mode='rgb_array')
                        elif self.render == 'human':
